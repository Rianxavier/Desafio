...
const faturamento: FaturamentoDiario[] = [
...
const analyzeBilling = (dados: FaturamentoDiario[]) => {
  ...
  return [
    `Menor faturamento do mês: R$ ${menorFaturamento.toFixed(2)}`,
    `Maior faturamento do mês: R$ ${maiorFaturamento.toFixed(2)}`,
    `Número de dias com faturamento acima da média: ${diasAcimaDaMedia} dias`,
  ];
};

console.log(analyzeBilling(faturamento));
